...
import com.mygoll.funform.core.Box
import com.mygoll.funform.core.Field
import com.mygoll.funform.core.Diagnostics
import com.mygoll.funform.core.Choice
import com.mygoll.funform.core.WindowRule
import com.mygoll.funform.core.Llm
import com.mygoll.funform.core.Engine
import com.mygoll.funform.core.Session
import java.io.File
 ...
 */
class FunFormService : AccessibilityService() {
    ...
    private fun preencher() {
        ...
        bolha = Bubble(
            ...
            aoDescartar = {
                painel?.fechar()
                abortarRodadaSeAtiva("você descartou a bolha")
            },
    ...
    private fun descreverRolavel(
        ...
        val sufixo = "· $campos campos · +$escolhasNovas escolhas · $nos nós · espera ${Pace.POS_ROLAGEM_MS}ms"
        if (no == null) return "varredura ${laco.size}: NENHUM rolável visível $sufixo"
        val r = Rect().also { no.getBoundsInScreen(it) }
        val id = no.viewIdResourceName ?: "sem id"
        return "varredura ${laco.size}: rolável ${no.className} ($id) ${r.width()}x${r.height()} $sufixo"
    }
    ...
    private val vigiaDoLaco = Runnable {
        ...
        laco.add("VIGIA: nenhum passo em ${TETO_SEM_PASSO_MS}ms, encerrando por segurança")
        encerrarRodada("o laço parou de responder (nenhum passo em ${TETO_SEM_PASSO_MS / 1000}s)")
    }
        ...
        when (val passo = m.proximoPasso()) {
            ...
            is Engine.Passo.Fim -> {
                ...
                if (m.telaNaoSeMoveu && !usandoGesto) {
                    ...
                    laco.add("a ação da árvore não moveu a tela: trocando para gesto de arrastar")
                    rolar(m)
                } else {
    ...
    private fun rolar(m: Engine) {
        if (!usandoGesto) {
            ...
            if (!pediu) {
                ...
                usandoGesto = true
                laco.add("a árvore recusou ACTION_SCROLL_FORWARD: trocando para gesto")
                rolar(m)
                return
            }
    ...
    private fun encerrarRodada(motivo: String) {
        ...
        if (!m.achouAlgumCampo) {
            ...
            Notices.texto(
                this,
                if (cegos > 0) "Nenhum campo de texto aqui, mas vi $cegos de escolha que ainda não opero."
                else "Não achei campo de texto nesta tela."
            )
            bolha?.fechar()
            bolha = null
            return
        ...
        val falhou = motivo.contains("parou de responder") || motivo.contains("não se move")
        bolha?.estado(
            when {
        ...
        painel = Panel(
            ...
            aoUsar = { chave ->
                ...
                val ok = sug != null && escrever(nosPorChave[chave], sug.resposta) &&
                    s.escreverAgora(chave, sug.resposta, fonte = "IA · aprovado por você")
                if (ok) {
            ...
            estadoAoVivo = {
                ...
                buildList {
                    ...
                    val m = motor
                    if (m != null) {
                        add("passo atual: ${m.proximoPasso()::class.simpleName}")
                        add("voltas de rolagem: ${m.voltasDeRolagem}")
                        add("rolando por: ${if (usandoGesto) "gesto de arrastar" else "ação da árvore"}")
                    } else if (motivoDaParada.isNotBlank()) {
                        add("parou porque: $motivoDaParada")
                    }
                }
            },
        ).also { it.mostrar() }
    ...
    private fun aoMudarJanela(event: AccessibilityEvent) {
        ...
        abortarRodadaSeAtiva("a tela mudou de app no meio da rodada")
