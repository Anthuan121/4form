...
class Panel(
    ...
    fun mostrar() {
        ...
        cartao.addView(service.kicker("Preenche · nesta página"))
        ...
        cartao.addView(
            service.botaoPrimario("Tentar de novo no que faltou") {
                fechar()
                aoTentarDeNovo()
            }
        )
        cartao.addView(service.botaoSecundario("Fechar") { fechar() })
        ...
        if (deu.isFailure) {
            Notices.texto(service, "O painel não conseguiu abrir: ${deu.exceptionOrNull()?.message}")
            aoFechar()
            return
        }
        raiz = root
    ...
    private fun montar() {
        ...
        titulo.text = buildString {
            append("Preenchi ${preenchidos.size} · ${abertos.size} em aberto")
            if (escolhas > 0) append(" · $escolhas de escolha (ainda não opero)")
        }
        ...
        if (passos.isNotEmpty()) {
            lista.addView(secao("O que está acontecendo"))
        ...
        if (abertos.isNotEmpty()) lista.addView(secao("Ficou com você"))
        for (r in abertos) {
            ...
            lista.addView(
                service.itemComPonto(r.rotulo ?: "campo sem nome", r.motivo ?: "", R.color.aviso)
            )
            ...
            if (consultando(chave)) {
                lista.addView(texto("perguntando à IA…", R.color.texto_secundario))
            }
            val caixa = campoTexto("digite o valor e toque em Escrever").apply {
            ...
            lista.addView(linhaDeBotoes(botaoCompacto("Escrever no campo", primario = true) {
                ...
                if (v.isNotBlank()) {
                    if (aoEscrever(chave, v)) {
                        ...
                        caixa.error = "não consegui escrever: o campo saiu da tela?"
                    }
                }
            }))
        }
        ...
        if (naoMarcadas > 0) {
            lista.addView(
                service.itemComPonto(
                    "Deixei $naoMarcadas escolha(s) em branco",
                    "É decisão do momento, não do perfil · marcar por você seria inventar.",
                    R.color.aviso,
                )
            )
        }
    ...
    private fun cartaoIa(chave: String, sug: Llm.Sugestao): LinearLayout = LinearLayout(service).apply {
        ...
        addView(service.kicker("Sugestão da IA · confiança ${sug.confianca}"))
        ...
        addView(linhaDeBotoes(
            botaoCompacto("Usar", primario = true) {
                if (aoUsar(chave)) montar()
                else Notices.texto(service, "não consegui escrever: o campo saiu da tela?")
            },
            botaoCompacto("Editar", primario = false) {
                emEdicao.add(chave)
                montar()
            },
            botaoCompacto("Descartar", primario = false) {
                aoDescartar(chave)
                montar()
            },
        ))
