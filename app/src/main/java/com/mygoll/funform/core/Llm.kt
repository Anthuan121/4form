...
object Llm {
    ...
    fun candidato(r: Session.Registro): Boolean =
        r.acao == "aberto" &&
            ...
            r.motivo != "o campo recusou a escrita" &&
            r.motivo != "você desfez"
    ...
    fun avaliar(resultadoHttp: Result<String>, linhasDePerfil: List<String>): Veredito {
        val bruto = resultadoHttp.getOrElse {
            return Veredito.NaoResponder("a IA não respondeu (${it.message ?: it.javaClass.simpleName})", doModelo = false)
        }
        return runCatching { avaliarConteudo(bruto, linhasDePerfil) }
            .getOrElse { Veredito.NaoResponder("resposta da IA ilegível", doModelo = false) }
    }
    ...
    private fun avaliarConteudo(bruto: String, linhas: List<String>): Veredito {
        ...
        val conteudo = lerString(bruto, "content", exigirFim = false)
            ?: return Veredito.NaoResponder("a IA respondeu sem conteúdo", doModelo = false)
        val i = conteudo.indexOf('{')
        if (i < 0) return Veredito.NaoResponder("a IA não devolveu o JSON combinado", doModelo = false)
        val json = conteudo.substring(i)
        val pode = Regex("\"pode_responder\"\\s*:\\s*(true|false)").find(json)?.groupValues?.get(1)
            ?: return Veredito.NaoResponder("a IA não devolveu o JSON combinado", doModelo = false)
        if (pode == "false") {
            val motivo = lerString(json, "resposta", exigirFim = true)?.takeIf { it.isNotBlank() }
            return Veredito.NaoResponder(motivo ?: "a IA disse que o seu perfil não tem esse dado", doModelo = true)
        }
        val resposta = lerString(json, "resposta", exigirFim = true)?.takeIf { it.isNotBlank() }
            ?: return Veredito.NaoResponder("a resposta da IA veio vazia ou cortada", doModelo = false)
        val ancora = lerString(json, "ancora", exigirFim = true)?.takeIf { it.isNotBlank() }
            ?: return Veredito.NaoResponder("resposta sem âncora no seu perfil: tratada como invenção", doModelo = false)
        if (!ancoraExiste(ancora, linhas)) {
            return Veredito.NaoResponder("a âncora citada não existe no seu perfil: tratada como invenção", doModelo = false)
        }
