...
class Engine(private val tetoDeVoltas: Int = TETO_DE_VOLTAS) {
    ...
    fun proximoPasso(): Passo = when {
        mudouDePacote -> Passo.Fim("a tela mudou de app no meio da rodada")
        ...
        !achouAlgumCampo && voltasDeRolagem == 0 -> Passo.Fim("não achei campo respondível nesta tela")
        telaNaoSeMoveu && !achouAlgumCampo -> Passo.Fim("rolei a tela inteira e não achei campo respondível")
        telaNaoSeMoveu -> Passo.Fim("rolei até o fim: a tela não se move mais")
        semRolagem -> Passo.Fim("a tela não rola mais")
        voltasDeRolagem >= tetoDeVoltas -> Passo.Fim("teto de $tetoDeVoltas rolagens: paro por segurança")
        else -> Passo.Rolar
    }
