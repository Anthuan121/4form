...
class Session(
    ...
    fun decidir(campo: Field): Decisao {
        if (campo.senha) return Decisao.DeixarAberto("campo de senha: nunca leio nem escrevo")
        if (!campo.textoAtual.isNullOrBlank()) {
            return Decisao.DeixarAberto("já tem texto: não sobrescrevo o que não fui eu que pus")
        }
        val r = Labeler.rotulo(campo, nivelPreferido)
            ?: return Decisao.DeixarAberto("não consegui identificar o campo")
        ...
        if (r.second == "viewId-cru") {
            return Decisao.DeixarAberto("não sei o que este campo pergunta")
        }
        val achado = perfil.valorPara(r.first)
            ?: return Decisao.DeixarAberto("\"${r.first}\": não tenho esse dado no seu perfil")
    ...
    fun decidirEscolha(escolha: Choice): Decisao {
        if (escolha.marcada) return Decisao.DeixarAberto("já estava marcada: não desmarco")
        if (!escolha.clicavel) return Decisao.DeixarAberto("a árvore não deixa clicar nesta opção")
        val rotulo = escolha.rotulo?.takeIf { it.isNotBlank() }
            ?: return Decisao.DeixarAberto("opção sem rótulo: não sei o que ela afirma")
        ...
        val achado = perfil.opcaoBateComPerfil(rotulo)
            ?: return Decisao.DeixarAberto("\"$rotulo\": seu perfil não declara isso")
        return Decisao.Preencher(achado.valor, achado.fonte)
    }
        ...
        when (decisao) {
            is Decisao.Preencher ->
                if (escreveu) {
                    ...
                    reg.motivo = "o campo recusou a escrita"
                }
     ...
     */
    fun escreverAgora(chave: String, valor: String, fonte: String = "você, no painel"): Boolean {
        val reg = porChave[chave] ?: return false
